...
# Data source: the full-altitude padded grid (includes zero-cost levels
# outside the contrail band so CompleteFlight trajectories don't exploit
# linear extrapolation). Generated by debug/convergence/generate_full_alt_grid.py.
DATA_PATH = (
    Path(__file__).resolve().parents[1]
    / "convergence"
    / "data"
    / "grid_era5_smoothed_full_alt.parquet.gz"
)
...
# Per-route bspline cache directory (kept alongside the input grid).
# One .casadi file per route, reused across phases and configs.
BSPLINE_CACHE_DIR = Path(__file__).parent / "bspline_cache"
...
def run_fuel_warmstart(
    ...
    """
    optimizer = phase_cls(AIRCRAFT, origin, destination, M0)
    optimizer.setup(debug=False, max_iter=1500)
...
def main() -> int:
    ...
    for origin, destination in ROUTES:
        ...
        cache_path = BSPLINE_CACHE_DIR / f"{route_label}.casadi"
        print(f"  interpolant cache: {cache_path}", flush=True)
        t_interp = time.time()
        interpolant = top.tools.cached_interpolant_from_dataframe(
            df_slice, cache_path, shape="bspline"
        )
        print(f"  interpolant ready in {time.time() - t_interp:.1f} s", flush=True)
